...
const ConfettiPiece: React.FC<{ initialX: number; initialY: number; rotation: number; delay: number; color: string }> = ({ initialX, initialY, rotation, delay, color }) => {
  ...
  // TypeScript's strict type checking for CSS properties does not recognize custom properties (e.g., '--initial-x').
  // This type assertion is a common workaround to bypass this check.
  const style = {
